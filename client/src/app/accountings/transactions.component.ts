...
export class TransactionsComponent implements OnInit{
  ...
  payment:any =['cash','credit','cash&credit']; 
...
/*determineAccount(account, payment){
  var ac:string ='';
   this.aservice.determineAccount(account,'debit',payment).subscribe(cAccounts => {
    ac = cAccounts['results']['rows'][0]['determine_account']; 
...
/*    this.service.addTransaction(this.param)                    
 ...
 filterChanged(selectedValue:string){ 
  ...
  addLadger(account:number, amount:number, cashAmount:number){
    this.journal = new Journal();
